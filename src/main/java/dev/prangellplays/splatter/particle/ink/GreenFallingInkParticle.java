...
public class GreenFallingInkParticle extends SpriteBillboardParticle {
    public GreenFallingInkParticle(ClientWorld clientWorld, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
    ...
    public void tick() {
        ...
        if (!this.dead && (this.onGround || this.isInFluid())) {
            this.markDead();
            this.world.addParticle(SplatterParticles.GREEN_INK_SPLAT, (double)((float)this.x), (double)((float)this.y + 0.01F), (double)((float)this.z), 0.0, 0.0, 0.0);
        }
    }
    ...
    public static class Factory implements ParticleFactory<DefaultParticleType> {
        ...
        public @Nullable Particle createParticle(DefaultParticleType parameters, ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
            GreenFallingInkParticle particle = new GreenFallingInkParticle(world, x, y, z, velocityX, velocityY, velocityZ);
