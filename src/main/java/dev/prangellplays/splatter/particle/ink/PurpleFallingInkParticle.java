...
public class PurpleFallingInkParticle extends SpriteBillboardParticle {
    public PurpleFallingInkParticle(ClientWorld clientWorld, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
    ...
    public void tick() {
        ...
        if (!this.dead && (this.onGround || this.isInFluid())) {
            this.markDead();
            this.world.addParticle(SplatterParticles.PURPLE_INK_SPLAT, (double)((float)this.x), (double)((float)this.y + 0.01F), (double)((float)this.z), 0.0, 0.0, 0.0);
        }
    }
    ...
    public static class Factory implements ParticleFactory<DefaultParticleType> {
        ...
        public @Nullable Particle createParticle(DefaultParticleType parameters, ClientWorld world, double x, double y, double z, double velocityX, double velocityY, double velocityZ) {
            PurpleFallingInkParticle particle = new PurpleFallingInkParticle(world, x, y, z, velocityX, velocityY, velocityZ);
