...
public class BlackInksplosion {
    ...
    public BlackInksplosion(World world, @Nullable Entity entity, double x, double y, double z, float power) {
        this.world = world;
        this.entity = entity;
        this.power = power;
        this.x = x;
    ...
    private void affectPos(BlockPos.Mutable blockPos) {
        ...
        for(int var11 = 0; var11 < var10; ++var11) {
            ...
            if (BlackInkBlock.canGrowOn(this.world, direction.getOpposite(), blockPos, blockState) && this.world.raycast(new RaycastContext(Vec3d.ofCenter(blockPos), this.pos, RaycastContext.ShapeType.COLLIDER, RaycastContext.FluidHandling.NONE, this.entity)).getBlockPos().equals(this.entity.getBlockPos())) {
                ...
                if (blockStateToInk.isOf(SplatterBlocks.BLACK_INK)) {
                    this.world.setBlockState(blockPos, (BlockState)blockStateToInk.with(BlackInkBlock.getProperty(direction.getOpposite()), true));
                } else if (blockStateToInk.isAir()) {
                    this.world.setBlockState(blockPos, (BlockState)SplatterBlocks.BLACK_INK.getDefaultState().with(BlackInkBlock.getProperty(direction.getOpposite()), true));
                }
            }
