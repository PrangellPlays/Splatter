package dev.prangellplays.splatter.components.octoling;
...
import dev.prangellplays.splatter.world.MagentaInksplosion;
...
public class PlayerMagentaOctolingComponent implements AutoSyncedComponent, CommonTickingComponent {
    public static final Identifier OCTOLING_TEXTURE = Splatter.id("textures/entity/octoling/magenta_octoling_base.png");
    ...
    public PlayerMagentaOctolingComponent(PlayerEntity player) {
    ...
    private void sync() {
        SplatterComponents.MAGENTA_OCTOLING.sync(this.player);
    }

    public boolean isOctoling() {
        return this.octoling;
    }

    private void correctOctoling() {
        boolean shouldBeOctoling = this.player.hasStatusEffect(SplatterStatusEffects.MAGENTA_INKSKIN);
        if (this.octoling != shouldBeOctoling) {
            this.octoling = shouldBeOctoling;
            this.sync();
        }
    ...
    public void setDiving(boolean diving) {
        ...
        if (var4 instanceof ServerWorld serverWorld) {
            ...
            while(var9.hasNext()) {
                ...
                boolean isSplash = Splatter.isCollidingWithMagentaInk(this.player);
    ...
    public void readFromNbt(NbtCompound nbt) {
        ...
        this.octoling = nbt.getBoolean("Octoling");
    }
    ...
    public void writeToNbt(NbtCompound nbt) {
        ...
        nbt.putBoolean("Octoling", this.octoling);
    }
    ...
    public void clientTick() {
        this.tick();
        if (this.isOctoling()) {
            boolean collidingWithFuneralInk = Splatter.isCollidingWithMagentaInk(this.player);
            boolean collidingWithFuneralStrict = Splatter.isCollidingWithMagentaInkStrict(this.player);
            if (this.isDiving()) {
                if (!collidingWithFuneralInk && this.player.isOnGround()) {
                    this.player.getWorld().addParticle(SplatterParticles.MAGENTA_INK_SPLAT, this.player.getParticleX(0.3), this.player.getY() + 0.009999999776482582, this.player.getParticleZ(0.3), (double)((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F), (double)(this.player.getWorld().random.nextFloat() * 0.3F), (double)((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F));
                }
            } else if (!collidingWithFuneralStrict && !this.player.isTouchingWater() && this.player.isOnGround()) {
                this.player.getWorld().addParticle(SplatterParticles.MAGENTA_FALLING_INK, this.player.getParticleX(0.3), this.player.getY() + 0.009999999776482582, this.player.getParticleZ(0.3), 0.0, 0.0, 0.0);
            }
        }
        ...
        if (this.isOctoling()) {
            ...
            if (this.isDiving()) {
                ...
                if (this.isMovingWhileDiving()) {
                    MagentaInksplosion inksplosion = new MagentaInksplosion(this.player.getWorld(), this.player, this.player.getX(), this.player.getY(), this.player.getZ(), 1.0F);
                    inksplosion.tick();
                ...
                if (Splatter.isCollidingWithMagentaInk(this.player)) {
                    this.setOutOfInkTimer(0);
    ...
    public enum Type {
        DEFAULT("default", false, (String)null, "textures/entity/octoling/magenta_octoling_crown_base");
