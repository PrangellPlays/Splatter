...
import dev.prangellplays.splatter.world.LightGrayInksplosion;
...
public class PlayerLightGrayOctolingComponent implements AutoSyncedComponent, CommonTickingComponent {
    public static final Identifier INKSKIN_TEXTURE = Splatter.id("textures/entity/inkskin/light_gray_octoling_base.png");
    ...
    public PlayerLightGrayOctolingComponent(PlayerEntity player) {
        this.player = player;
    }

    private void sync() {
        SplatterComponents.LIGHT_GRAY_OCTOLING.sync(this.player);
    }
    ...
    private void correctOctoling() {
        boolean shouldBeOctoling = this.player.hasStatusEffect(SplatterStatusEffects.LIGHT_GRAY_INKSKIN);
    ...
    public void setDiving(boolean diving) {
        ...
        if (var4 instanceof ServerWorld serverWorld) {
            ...
            while(var9.hasNext()) {
                ...
                boolean isSplash = Splatter.isCollidingWithLightGrayInk(this.player);
    ...
    public void clientTick() {
        this.tick();
        if (this.isOctoling()) {
            boolean collidingWithLightGrayInk = Splatter.isCollidingWithLightGrayInk(this.player);
            if (this.isDiving()) {
                if (!collidingWithLightGrayInk && this.player.isOnGround()) {
            ...
            } else if (!collidingWithLightGrayInk && !this.player.isTouchingWater() && this.player.isOnGround()) {
        ...
        if (this.isOctoling()) {
            ...
            if (this.isDiving()) {
                ...
                if (this.isMovingWhileDiving()) {
                    LightGrayInksplosion inksplosion = new LightGrayInksplosion(this.player.getWorld(), this.player, this.player.getX(), this.player.getY(), this.player.getZ(), SplatterCommand.getInkSpreadPower());
                    inksplosion.tick();
                ...
                if (Splatter.isCollidingWithLightGrayInk(this.player)) {
                    this.setOutOfInkTimer(0);
