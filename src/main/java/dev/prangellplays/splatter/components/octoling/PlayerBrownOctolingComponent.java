...
import dev.prangellplays.splatter.world.BrownInksplosion;
...
public class PlayerBrownOctolingComponent implements AutoSyncedComponent, CommonTickingComponent {
    public static final Identifier OCTOLING_TEXTURE = Splatter.id("textures/entity/octoling/brown_octoling_base.png");
    ...
    public PlayerBrownOctolingComponent(PlayerEntity player) {
    ...
    private void sync() {
        SplatterComponents.BROWN_OCTOLING.sync(this.player);
    }
    ...
    private void correctOctoling() {
        boolean shouldBeOctoling = this.player.hasStatusEffect(SplatterStatusEffects.BROWN_INKSKIN);
    ...
    public void setDiving(boolean diving) {
        ...
        if (var4 instanceof ServerWorld serverWorld) {
            ...
            while(var9.hasNext()) {
                ...
                boolean isSplash = Splatter.isCollidingWithBrownInk(this.player);
    ...
    public void clientTick() {
        this.tick();
        if (this.isOctoling()) {
            boolean collidingWithFuneralInk = Splatter.isCollidingWithBrownInk(this.player);
            boolean collidingWithFuneralStrict = Splatter.isCollidingWithBrownInkStrict(this.player);
            if (this.isDiving()) {
                if (!collidingWithFuneralInk && this.player.isOnGround()) {
                    this.player.getWorld().addParticle(SplatterParticles.BROWN_INK_SPLAT, this.player.getParticleX(0.3), this.player.getY() + 0.009999999776482582, this.player.getParticleZ(0.3), (double)((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F), (double)(this.player.getWorld().random.nextFloat() * 0.3F), (double)((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F));
                }
            } else if (!collidingWithFuneralStrict && !this.player.isTouchingWater() && this.player.isOnGround()) {
                this.player.getWorld().addParticle(SplatterParticles.BROWN_FALLING_INK, this.player.getParticleX(0.3), this.player.getY() + 0.009999999776482582, this.player.getParticleZ(0.3), 0.0, 0.0, 0.0);
            }
        }
        ...
        if (this.isOctoling()) {
            ...
            if (this.isDiving()) {
                ...
                if (this.isMovingWhileDiving()) {
                    BrownInksplosion inksplosion = new BrownInksplosion(this.player.getWorld(), this.player, this.player.getX(), this.player.getY(), this.player.getZ(), 1.0F);
                    inksplosion.tick();
                ...
                if (Splatter.isCollidingWithBrownInk(this.player)) {
                    this.setOutOfInkTimer(0);
    ...
    public enum Type {
        DEFAULT("default", false, (String)null, "textures/entity/octoling/brown_octoling_crown_base");
