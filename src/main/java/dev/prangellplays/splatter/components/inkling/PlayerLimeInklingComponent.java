...
public class PlayerLimeInklingComponent implements AutoSyncedComponent, CommonTickingComponent {
    public static final Identifier INKSKIN_TEXTURE = Splatter.id("textures/entity/inkskin/lime_inkling_base.png");
    ...
    public void clientTick() {
        this.tick();
        if (this.isInkling()) {
            boolean collidingWithLimeInk = Splatter.isCollidingWithLimeInk(this.player);
            if (this.isDiving()) {
                if (!collidingWithLimeInk && this.player.isOnGround()) {
                    this.player.getWorld().addParticle(SplatterParticles.LIME_INK_SPLAT, this.player.getParticleX(0.3), this.player.getY() + 0.009999999776482582, this.player.getParticleZ(0.3), (double) ((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F), (double) (this.player.getWorld().random.nextFloat() * 0.3F), (double) ((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F));
                }
            } else if (!collidingWithLimeInk && !this.player.isTouchingWater() && this.player.isOnGround()) {
        ...
        if (this.isInkling()) {
            ...
            if (this.isDiving()) {
                ...
                if (this.isMovingWhileDiving()) {
                    LimeInksplosion inksplosion = new LimeInksplosion(this.player.getWorld(), this.player, this.player.getX(), this.player.getY(), this.player.getZ(), SplatterCommand.getInkSpreadPower());
                    inksplosion.tick();
