...
import dev.prangellplays.splatter.world.YellowInksplosion;
...
public class PlayerYellowInklingComponent implements AutoSyncedComponent, CommonTickingComponent {
    public static final Identifier INKLING_TEXTURE = Splatter.id("textures/entity/inkling/yellow_inkling_base.png");
    ...
    public PlayerYellowInklingComponent(PlayerEntity player) {
    ...
    private void sync() {
        SplatterComponents.YELLOW_INKLING.sync(this.player);
    }
    ...
    private void correctInkling() {
        boolean shouldBeInkling = this.player.hasStatusEffect(SplatterStatusEffects.YELLOW_INKSKIN);
    ...
    public void setDiving(boolean diving) {
        ...
        if (var4 instanceof ServerWorld serverWorld) {
            ...
            while(var9.hasNext()) {
                ...
                boolean isSplash = Splatter.isCollidingWithYellowInk(this.player);
    ...
    public void clientTick() {
        this.tick();
        if (this.isInkling()) {
            boolean collidingWithFuneralInk = Splatter.isCollidingWithYellowInk(this.player);
            boolean collidingWithFuneralStrict = Splatter.isCollidingWithYellowInkStrict(this.player);
            if (this.isDiving()) {
                if (!collidingWithFuneralInk && this.player.isOnGround()) {
                    this.player.getWorld().addParticle(SplatterParticles.YELLOW_INK_SPLAT, this.player.getParticleX(0.3), this.player.getY() + 0.009999999776482582, this.player.getParticleZ(0.3), (double)((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F), (double)(this.player.getWorld().random.nextFloat() * 0.3F), (double)((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F));
                }
            } else if (!collidingWithFuneralStrict && !this.player.isTouchingWater() && this.player.isOnGround()) {
                this.player.getWorld().addParticle(SplatterParticles.YELLOW_FALLING_INK, this.player.getParticleX(0.3), this.player.getY() + 0.009999999776482582, this.player.getParticleZ(0.3), 0.0, 0.0, 0.0);
            }
        }
        ...
        if (this.isInkling()) {
            ...
            if (this.isDiving()) {
                ...
                if (this.isMovingWhileDiving()) {
                    YellowInksplosion inksplosion = new YellowInksplosion(this.player.getWorld(), this.player, this.player.getX(), this.player.getY(), this.player.getZ(), 1.0F);
                    inksplosion.tick();
                ...
                if (Splatter.isCollidingWithYellowInk(this.player)) {
                    this.setOutOfInkTimer(0);
    ...
    public enum Type {
        DEFAULT("default", false, (String)null, "textures/entity/inkling/yellow_inkling_crown_base");
