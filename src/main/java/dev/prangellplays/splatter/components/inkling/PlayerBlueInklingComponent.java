...
public class PlayerBlueInklingComponent implements AutoSyncedComponent, CommonTickingComponent {
    public static final Identifier INKLING_TEXTURE = Splatter.id("textures/entity/inkling/blue_inkling_base.png");
    ...
    public void clientTick() {
        this.tick();
        if (this.isInkling()) {
            boolean collidingWithFuneralInk = Splatter.isCollidingWithBlueInk(this.player);
            boolean collidingWithFuneralStrict = Splatter.isCollidingWithBlueInkStrict(this.player);
            if (SplatterConfig.inkParticles) {
                if (this.isDiving()) {
                    if (!collidingWithFuneralInk && this.player.isOnGround()) {
                        this.player.getWorld().addParticle(SplatterParticles.BLUE_INK_SPLAT, (double) ((float) this.player.getX()), (double) ((float) this.player.getY() + 0.01F), (double) ((float) this.player.getZ()), 0.0, 0.0, 0.0);
                    }
        ...
        if (this.isInkling()) {
            ...
            if (this.isDiving()) {
                ...
                if (this.isMovingWhileDiving()) {
                    BlueInksplosion inksplosion = new BlueInksplosion(this.player.getWorld(), this.player, this.player.getX(), this.player.getY(), this.player.getZ(), 1.0F);
                    inksplosion.tick();
    ...
    public enum Type {
        DEFAULT("default", false, (String)null, "textures/entity/inkling/blue_inkling_crown_base");
