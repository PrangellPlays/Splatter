...
import dev.prangellplays.splatter.world.OrangeInksplosion;
...
public class PlayerOrangeInklingComponent implements AutoSyncedComponent, CommonTickingComponent {
    public static final Identifier INKLING_TEXTURE = Splatter.id("textures/entity/inkling/orange_inkling_base.png");
    ...
    public PlayerOrangeInklingComponent(PlayerEntity player) {
    ...
    private void sync() {
        SplatterComponents.ORANGE_INKLING.sync(this.player);
    }
    ...
    private void correctInkling() {
        boolean shouldBeInkling = this.player.hasStatusEffect(SplatterStatusEffects.ORANGE_INKSKIN);
    ...
    public void setDiving(boolean diving) {
        ...
        if (var4 instanceof ServerWorld serverWorld) {
            ...
            while(var9.hasNext()) {
                ...
                boolean isSplash = Splatter.isCollidingWithOrangeInk(this.player);
    ...
    public void clientTick() {
        this.tick();
        if (this.isInkling()) {
            boolean collidingWithFuneralInk = Splatter.isCollidingWithOrangeInk(this.player);
            boolean collidingWithFuneralStrict = Splatter.isCollidingWithOrangeInkStrict(this.player);
            if (this.isDiving()) {
                if (!collidingWithFuneralInk && this.player.isOnGround()) {
                    this.player.getWorld().addParticle(SplatterParticles.ORANGE_INK_SPLAT, this.player.getParticleX(0.3), this.player.getY() + 0.009999999776482582, this.player.getParticleZ(0.3), (double)((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F), (double)(this.player.getWorld().random.nextFloat() * 0.3F), (double)((this.player.getWorld().random.nextFloat() - 0.5F) * 0.3F));
                }
            } else if (!collidingWithFuneralStrict && !this.player.isTouchingWater() && this.player.isOnGround()) {
                this.player.getWorld().addParticle(SplatterParticles.ORANGE_FALLING_INK, this.player.getParticleX(0.3), this.player.getY() + 0.009999999776482582, this.player.getParticleZ(0.3), 0.0, 0.0, 0.0);
            }
        }
        ...
        if (this.isInkling()) {
            ...
            if (this.isDiving()) {
                ...
                if (this.isMovingWhileDiving()) {
                    OrangeInksplosion inksplosion = new OrangeInksplosion(this.player.getWorld(), this.player, this.player.getX(), this.player.getY(), this.player.getZ(), 1.0F);
                    inksplosion.tick();
                ...
                if (Splatter.isCollidingWithOrangeInk(this.player)) {
                    this.setOutOfInkTimer(0);
    ...
    public enum Type {
        DEFAULT("default", false, (String)null, "textures/entity/inkling/orange_inkling_crown_base");
