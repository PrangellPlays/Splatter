package dev.prangellplays.splatter.mixin.server;
...
public class TooFastServerPlayNetworkHandlerMixin {
    ...
    private float splatter$PlayerMaxSpeed(float speed) {
        return Float.MAX_VALUE;
    }

    @ModifyConstant(method = "onPlayerMove", constant = @Constant(floatValue = 300.0F))
    private float splatter$ElytraMaxSpeed(float speed) {
        return Float.MAX_VALUE;
    }

    @ModifyConstant(method = "onVehicleMove", constant = @Constant(doubleValue = 100.0))
    private double splatter$VehicleMaxSpeed(double speed) {
