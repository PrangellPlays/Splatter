...
public class InkTotemYellowItem extends Item {
    public InkTotemYellowItem(Settings settings) {
        super(settings);
    }

    @Override
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.hasStatusEffect(SplatterStatusEffects.YELLOW_INKSKIN)) {
            user.addStatusEffect(new StatusEffectInstance(SplatterStatusEffects.YELLOW_INKSKIN, 24000, 0, false, false));
