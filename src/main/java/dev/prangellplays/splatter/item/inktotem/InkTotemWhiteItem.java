...
public class InkTotemWhiteItem extends Item {
    public InkTotemWhiteItem(Settings settings) {
        super(settings);
    }

    @Override
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!user.hasStatusEffect(SplatterStatusEffects.WHITE_INKSKIN)) {
            user.addStatusEffect(new StatusEffectInstance(SplatterStatusEffects.WHITE_INKSKIN, 24000, 0, false, false));
